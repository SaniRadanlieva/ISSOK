...
function setSessionCookie($username){
    session_start();
    $value = session_id() . '|' . time() . '|' . $username;
    setSessionCookie('session_cookie', $value, time() + 60 * 60);
    //last access
}
...
if($_POST) {
    ...
    $zapamtiMe = isset($_POST['zapamtiMe']);
...
if (isset($_COOKIE['session_cookie'])) {
    ...
    if ($currentTime - $lastAccessTime < 7200) { //2*60*60
        $output .= "<p class='output'>Hello $storedUsername!</p>";
    }

    if ($currentTime - $lastAccessTime > 3600) { //60*60
...
<form action="" method="post" id="register-form" novalidate="novalidate">
    ...
    <div class="label">Zapamti me</div>
    <input type="checkbox" id="zapamtiMe" name="zapamtiMe" <?php if ($zapamtiMe) echo 'checked'; ?> />
